...
Nodes represent variable bindings at specific source locations (SSA-style).
Edges represent data flow between bindings.
"""
...
class EdgeKind(Enum):
    ASSIGN = auto()       # x = y
    PARAM_PASS = auto()   # foo(x) -> param
    RETURN = auto()        # return x
    IMPORT = auto()        # from mod import x
    ATTR_ACCESS = auto()   # x.attr
    REASSIGN = auto()      # x = ...; later x = ... (links old to new)
    CALL_ARG = auto()      # passed as argument to a call
...
class FlowNode:
    ...
    loc: Location
    kind: NodeKind
    scope: str  # dotted scope path: "module.Class.method"
    is_external: bool = False  # True for stdlib/third-party imports
...
class FlowEdge:
    ...
    dst: str  # node id
    kind: EdgeKind
    transform: str | None = None       # e.g. "encode", "sha256"
    transform_category: str | None = None  # e.g. "encoding", "hashing"
    sink: str | None = None            # e.g. "print", "execute"
    sink_category: str | None = None   # e.g. "console", "database"
...
class FlowGraph:
    ...
    def chain_forward(self, node_id: str, max_depth: int = 50) -> list[list[FlowNode]]:
        """All forward paths from a node (DFS)."""
    ...
    def chain_backward(self, node_id: str, max_depth: int = 50) -> list[list[FlowNode]]:
        """All backward paths to a node (DFS)."""
        chains: list[list[FlowNode]] = []
        visited: set[str] = set()
